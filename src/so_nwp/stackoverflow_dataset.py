...
def create_vocab(vocab_size):
    ...
    vocab_dict = stackoverflow.load_word_counts('./datasets')
...
def construct_word_level_datasets(vocab_size: int,
    ...
    (stackoverflow_train, _,
     stackoverflow_test) = stackoverflow.load_data('./datasets')
...
def get_centralized_datasets(vocab_size: int,
    ...
    to_ids = build_to_ids_fn(
        vocab=vocab, max_seq_len=max_seq_len, num_oov_buckets=num_oov_buckets)
    raw_train, _, raw_test = stackoverflow.load_data(cache_dir='./datasets')
    ...
    # client_datasets = []
    # for i, x in enumerate(raw_test.datasets(seed=None)):
    #     if i == 1000:
    #         break
    #     client_datasets.append(x)
    # #client_datasets = [d for d in raw_test.datasets(seed=None)]
    # nested_dataset = tf.data.Dataset.from_tensor_slices(client_datasets)
    # test_dataset = nested_dataset.flat_map(lambda x: x)
